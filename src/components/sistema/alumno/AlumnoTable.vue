  ...
  <div
    ...
    <div class="flex flex-col">
      <div class="grid grid-cols-6 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-6 text-center text-xs">
        ...
        <div class="p-2.5 xl:p-5">
          <h5 class="uppercase">Número documento</h5>
        </div>
        <div class="p-2.5 xl:p-5">
          <h5 class="uppercase">Apellidos</h5>
        </div>
        <div class="p-2.5 xl:p-5">
          <h5 class="uppercase">Nombres</h5>
        </div>
        <div class="p-2.5 xl:p-5">
      ...
      <div v-if="alumnos.length === 0" class="flex justify-center py-6 text-gray-500 dark:text-gray-300 text-xs">
        No se encontraron alumnos.
      </div>
      <div v-else v-for="(alumno, index) in alumnos" :key="alumno.id"
        :class="`grid grid-cols-6 sm:grid-cols-6 items-center text-xs ${index < alumnos.length - 1 ? 'border-b border-stroke dark:border-strokedark' : ''}`">
        <div class="p-2.5 xl:p-5 flex items-center justify-start">
          <p class="text-black dark:text-white">
            {{ alumno.tipoDocumento?.abreviatura || 'Sin tipo documento' }}
          </p>
        </div>
        <div class="p-2.5 xl:p-5 flex items-center justify-start">
          <p class="text-black dark:text-white">{{ alumno.numero_documento }}</p>
        </div>
        <div class="p-2.5 xl:p-5 flex items-center justify-start">
          <p class="text-black dark:text-white">{{ alumno.apellido_paterno }} {{ alumno.apellido_materno }}</p>
        </div>
        <div class="p-2.5 xl:p-5 flex items-center justify-start">
          <p class="text-black dark:text-white">{{ alumno.nombres }}</p>
        </div>
...
const fetchAlumnos = async (page = 1, query = '') => {
  await alumnoStore.fetchAlumnosPaginate({ page, query });
};

const applySearch = () => {
  fetchAlumnos(1, searchInput.value.trim());
};
...
const currentPage = computed(() => alumnoStore.pagination.currentPage);
...
const toggleEstado = async () => {
  ...
  if (alumno) {
    ...
    fetchAlumnos(currentPage.value, searchInput.value.trim()); // Refrescar la tabla con la página actual
  }
};
